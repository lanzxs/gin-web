...
func InitLogger() {
	now := time.Now()
	filename := fmt.Sprintf("%s/%04d-%02d-%02d.log", config.LoggerSetting.Path, now.Year(), now.Month(), now.Day())
	hook := &lumberjack.Logger{
		Filename:   filename,             // 日志文件路径
		MaxSize:    config.LoggerSetting.MaxSize,    // 最大尺寸, M
		MaxBackups: config.LoggerSetting.MaxBackups, // 备份数
		MaxAge:     config.LoggerSetting.MaxAge,     // 存放天数
		Compress:   config.LoggerSetting.Compress,   // 是否压缩
	}
	defer hook.Close()
	...
	core := zapcore.NewCore(
		...
		zapcore.DebugLevel,                                                             // 日志等级
	)

	logger := zap.New(core, zap.AddCaller(), zap.AddCallerSkip(1))
	globar.Logger = logger.Sugar()
	globar.Logger.Debug("初始化日志完成")
}
